...
def find_keyword(keyword):
    db = DB()
    try:
        print(f"Searching for keyword {bold_keyword(keyword)}...")
        ...
        if keyword_record:
            ...
            if status == "complete":
                ...
                return f"<p>No results found for keyword: {bold_keyword(keyword)}.</p>"
                
            elif status == "running":
                return f"""
                <p>Searching for keyword: {bold_keyword(keyword)}. This could take a while...</p>
            ...
            return f"<p>Starting a keyword search for: {bold_keyword(keyword)}</p>"
...
def stop_crawl(keyword):
    ...
    return f"<p>Crawl for keyword '{bold_keyword(keyword)}' stopped.</p>"
...
def crawl_status(keyword):
    ...
    return f"""
    <p>URLs visited for keyword {bold_keyword(keyword)}: {visited_count}.</p>
